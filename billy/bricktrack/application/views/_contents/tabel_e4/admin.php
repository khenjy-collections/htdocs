...
<div id="card-view" class="row data-view active">
  <?php foreach ($tbl_e4->result() as $tl_e4):
    echo card_regular(
      ...
      card_content('tabel_e4_field3', $tl_e4->$tabel_e4_field3) .
      card_content('tabel_e4_field4', $tl_e4->$tabel_e4_field4) .
      card_content('tabel_e4_field5', $tl_e4->$tabel_e4_field5),
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <form action="<?= site_url($language . '/' . $tabel_e4 . '/tambah') ?>" method="post">
        <div class="modal-body">
          <div class="form-group">
            <select class="form-control float" required name="<?= $tabel_e4_field2_input ?>"
              id="<?= $tabel_e4_field2_input ?>">
              <?php foreach ($tbl_c1->result() as $tl_c1): ?>
                <option value="<?= $tl_c1->$tabel_c1_field1 ?>"><?= $tl_c1->$tabel_c1_field2 ?></option>
              <?php endforeach ?>
            </select>
            <label for="<?= $tabel_e4_field2_input ?>" class="form-label"><?= lang('select') ?>
              <?= $tabel_e4_field2_alias ?></label>
          </div>
          <?= add_min_max('date', 'tabel_e4_field3', 'required', date("Y-m-d"), '') ?>
          <?= add_min_max('time', 'tabel_e4_field4', 'required', '', '') ?>
          <?= add_min_max('time', 'tabel_e4_field5', 'required', '', '') ?>
        </div>
        <!-- memunculkan notifikasi modal -->
      ...
      <div class="modal-content">
        ...
        <form action="<?= site_url($language . '/' . $tabel_e4 . '/update') ?>" method="post"
          ...
          <div class="modal-body">
            ...
            <?= edit_min_max('date', 'tabel_e4_field4', $tl_e4->$tabel_e4_field4, 'required', date("Y-m-d"), '') ?>
            <?= edit_min_max('time', 'tabel_e4_field4', $tl_e4->$tabel_e4_field4, 'required', '', '') ?>
            <?= edit_min_max('time', 'tabel_e4_field5', $tl_e4->$tabel_e4_field5, 'required', '', '') ?>
          </div>
